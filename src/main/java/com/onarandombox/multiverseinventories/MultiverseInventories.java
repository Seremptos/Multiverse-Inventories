...
public class MultiverseInventories extends JavaPlugin implements MVPlugin, Messaging {
    ...
    public void convertPlayernamesToUUID() throws IOException {
            ...
            try (Stream<Path> stream = Files.walk(path)) {
                ...
                filesList.forEach(file -> {
                    ...
                    try {
                        ...
                        Logging.getLogger().info("Trying to convert user " + presumedUUID + " in folder "+file.getParent().toFile().getName()+" to UUID.");
                        OfflinePlayer player = Bukkit.getOfflinePlayer(presumedUUID);
                        if (player.getName() == null) {
                            Logging.getLogger().info("Can't find user " + presumedUUID + ", conversion aborted.");
                        }
                        if (file.toFile().renameTo(new File(file.getParent().toFile(), player.getUniqueId() + ".json"))) {
                            Logging.getLogger().info("Successfully renamed " + presumedUUID + " in folder "+file.getParent().toFile().getName()+" to " + player.getUniqueId() + ".");
                        } else {
                            Logging.getLogger().info("Failed renaming " + presumedUUID + " in folder "+file.getParent().toFile().getName()+" to " + player.getUniqueId() + ".");
                        }
                    }
                });
        ...
        public void onEnable() {
            ...
            try {
                ...
                Logging.getLogger().warning("Error while converting playernames to UUIDs.");
            }
